...
public class StreamClientAgent implements IStreamClientAgent {
	...
	public StreamClientAgent(StreamFrameListener streamFrameListener, Dimension dimension) {
		...
		this.clientBootstrap.setPipelineFactory(new StreamClientChannelPipelineFactory(streamClientListener, streamFrameListener, dimension));
	}
	...
	public void connect(SocketAddress streamServerAddress) {
		...
		ChannelFuture result = clientBootstrap.connect(streamServerAddress);
		if (!result.isSuccess()) {
			logger.warn("connect fails with server :{}", streamServerAddress);
		}
	}
