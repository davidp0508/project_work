...
public class StreamClient {
	...
	private final static String testip = "128.237.113.38";
	...
	private static NameIpPort [] ipArray = new NameIpPort[PLAYERS];
	...
	private static Integer [] scores = new Integer[PLAYERS];
	...
	//client No: each client is assigned a client No to identify.
	private static int clientNo;

	private static int actorNo = 0;

	public static void main(String[] args) {
		int winner;

		//build a messager to send msgs to other
		messager = MessageFactory.getMessager("client" + args[0], "local_server" + args[0] + "@" + testip, "test");

		clientNo = Integer.parseInt(args[0]);

		//three test case  
		ipArray[0] = new NameIpPort("hanyang", testip, 20000);
		ipArray[1] = new NameIpPort("hayang", testip, 20001);
		ipArray[2] = new NameIpPort("hyang", testip, 20002);

		displayWindow = new GameWindow(dimension, testip, clientNo, ipArray);
        ...
        if(clientNo == 0){
        	int option = JOptionPane.showConfirmDialog(null, ipArray[clientNo].hostname + " Do you want to be the actor?", "Confirm",
        ...
        Listener myServer = new Listener(ipArray[clientNo].hostname, testip, receivedMsgs, ipArray);
		...
		while(true){
			...
			if(rec.type.equals("ANSWER")){
				...
				if(state == ACTING){
					...
					if(reved_answer.equals(correct_answer)){
						...
						messager.multicast(clientNo, "SCOREUPDATE " + compressScores(), ipArray);
			...
			if(rec.type.equals("ROUND_FINISHED")){
				...
				displayWindow.changeActor((++actorNo)%3, dimension.height);
				currentsec = MAXSEC;
				stop = true;
				continue;
			}
	...
	private static void change2ACTING(){
		...
		messager.multicast(clientNo, "BEGIN ", ipArray);
		stop = false;		
	}
	...
	private static void change2GUESSING(){
		state = GUESSING;
		displayWindow.submitbutton.setEnabled(true);
		messager.multicast(clientNo, "ROUND_FINISHED " + " ", ipArray);
		displayWindow.changeActor((++actorNo)%3, dimension.height);
		messager.sendMsg("TOKEN " + " ", ipArray[(clientNo+1)%PLAYERS].hostname, 
                ipArray[(clientNo+1)%PLAYERS].ip);
		currentsec = MAXSEC;
		stop = true;
